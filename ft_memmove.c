...
/*   Updated: 2023/09/26 10:31:50 by sarcasti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h" // Incluimos el archivo de encabezado de "libft" para asegurarnos de tener acceso a las definiciones necesarias.
...
void	*ft_memmove(void *dest, const void *src, size_t n)
{
	...
	if (dest < src) // Si 'dest' está antes de 'src' en memoria (sin superposición).
	{
	...
	else // Si 'src' está antes o en la misma posición que 'dest' en memoria (posible superposición).
	{
		d = dest + (n - 1); // Apuntamos al último byte de 'dest'.
		s = src + (n - 1); // Apuntamos al último byte de 'src'.
		while (n--) // Copiamos 'n' bytes de 'src' a 'dest', de derecha a izquierda para evitar superposición.
			*d-- = *s--;
	}
